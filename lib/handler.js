    ...
    generateImage(type, options = {}) {
        return new Promise((resolve, reject) => {
            ...
            if (type = "awoo" && (options.hair || options.face)) {
                if (options.hair && !options.hair.match(/^[0-9A-Fa-f]{6}$/)) return reject(new Error("Hair must be a valid hex code"));
                if (options.face && !options.face.match(/^[0-9A-Fa-f]{6}$/)) return reject(new Error("Face must be a valid hex code"));
    ...
    generateLicense(options = {}) {
        return new Promise((resolve, reject) => {
            ...
            post(postOptions, function (error, response, body) {
                ...
                if (response.statusCode != 200) reject(JSON.parse(body));
                else resolve(body)
            });
        });
    }
