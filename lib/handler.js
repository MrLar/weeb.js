...
class Handler {
    ...
    generateLicense(options = {}) {
        return new Promise((resolve, reject) => {
            ...
            if (options.badges && (!(options.badges instanceof Array) || options.badges.length > 3 || options.badges.length < 1)) return reject(new TypeError("Badges must be an array that does contains 1 and not more than 3 values"))
            if (options.widgets && (!(options.widgets instanceof Array) || options.widgets.length > 3 || options.widgets.length < 1)) return reject(new TypeError("Widgets must be an array that does contains 1 and not more than 3 values"))
            const postData = {
                "title": options.title,
