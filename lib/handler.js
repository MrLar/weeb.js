...
class Handler {
    ...
    getVersion(type = "module") {
        return new Promise((resolve, reject) => {
            if(type != "module" && type != "images" && type != "generation") return reject(new Error("The version type to get must module, images or generation."));
            if(type === "module") resolve(version);
            let query = ENDPOINTS.INFO.GENERATION;
            if(type === "images") query = ENDPOINTS.INFO.IMAGES;
            https.request(Object.assign(url.parse(BASE_URL + query), {
                headers: {'Authorization': this.key, 'User-Agent': this.agent ? this.agent : "Weeb.js/"+version}
            }), res => {
                let chunked = '';
                ...
                res.on('end', () => {
                    try {
                        ...
                        if (ret.status !== 200) reject(ret);
                            ...
                            resolve([ret.url, ret.type+"-"+ret.id, ret.fileType]);
                        }
                    } catch (err) {
                        reject(err);
                    }
